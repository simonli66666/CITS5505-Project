...
def user():
    ...
    print("Requested username:", username)  # Debug output
    if username:
        other_user = User.query.filter_by(username=username).first()
        print("Found user:", other_user)  # Debug output
        if other_user:
            return render_template('frontend/others.html', user=other_user)
        else:
            print("User not found")  # Debug output
            return "<h1>User not found</h1>", 404
    else:
        return render_template('frontend/user.html', user=current_user)
